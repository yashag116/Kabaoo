...
export const Route = createFileRoute("/challenges/friend")({
  head: () => ({
...
function FriendChallengePage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!profile || profile.balance < feeInPaise) {
        throw new Error(`Insufficient funds. You need at least ${formatINR(currentFee)} in your account.`);
      }
      ...
      if (participantError) throw new Error("Failed to register you in the tournament.");
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    try {
      let searchCode = joinCode.toUpperCase().trim();
      if (!searchCode.startsWith("KABAOO-") && searchCode.length === 5) {
        searchCode = `KABAOO-${searchCode}`;
      }
      ...
      if (error || !match) {
        throw new Error("Invalid lobby code. The match may not exist or the code is wrong.");
      }
  ...
  return (
    <AppShell>
      <div className="max-w-3xl mx-auto space-y-8">
        <div>
          <h1 className="font-display text-3xl md:text-4xl font-bold tracking-tight">Private Match Making</h1>
          <p className="text-muted-foreground text-sm mt-1">
            Host a secure match or join your teammate's squad custom lobby using an access key.
          </p>
        </div>

        <Tabs defaultValue="create" className="w-full">
          <TabsList className="grid w-full grid-cols-2 mb-8">
            <TabsTrigger value="create" className="text-base font-semibold transition-all duration-300">Host Custom Arena</TabsTrigger>
          ...
          <TabsContent value="create" className="space-y-6">
            {!createdMatchId ? (
              <form onSubmit={handleCreate} className="rounded-lg border border-border bg-card p-6 md:p-8 shadow-[var(--shadow-card)] space-y-8 flex flex-col">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      <button type="button" onClick={() => navigate({ to: "/challenges/create" })} className="rounded-lg border border-border p-3 text-left transition-all duration-300 hover:border-primary/40 group card-lift">
                        <Globe2 className="size-4 text-muted-foreground group-hover:text-primary mb-1 transition-colors" />
                        <div className="font-display font-bold text-sm group-hover:text-primary transition-colors">Public Bracket</div>
                      </button>
                      <button type="button" className="rounded-lg border border-primary bg-primary/10 p-3 text-left transition-all duration-300 cursor-default shadow-[var(--shadow-glow)] card-lift scale-[1.02]">
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      className="w-full h-12 rounded-md border border-border bg-surface px-3 text-sm font-semibold focus:outline-none focus:border-primary transition-all duration-300 cursor-pointer card-lift hover:border-primary/40"
                    >
                ...
                <div>
                  ...
                  <Input
                    ...
                    className="mt-3 font-mono max-w-xs transition-all duration-300 hover:border-primary/40 focus-visible:border-primary"
                  />
                </div>
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    className="mt-1.5 transition-all duration-300 hover:border-primary/40 focus-visible:border-primary"
                    rows={2}
                  />
                </div>
                ...
                <div className="pt-2">
                  ...
                  >
                    ...
                    {createLoading ? "Generating Secure Credentials..." : "Submit Entry Fee & Open Private Bracket"}
                  </Button>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                <div className="relative overflow-hidden rounded-lg border border-primary/30 bg-card p-8 md:p-12 text-center shadow-[var(--shadow-glow)]">
                  <div className="absolute inset-0 bg-grid opacity-20" />
                  <div className="absolute -top-32 left-1/2 -translate-x-1/2 size-96 rounded-full bg-primary/10 blur-3xl pointer-events-none" />
                  
                  <div className="relative space-y-4">
                    <div className="text-xs uppercase tracking-[0.2em] text-primary font-bold flex justify-center items-center gap-2">
                      <CheckCircle2 className="size-4 text-success" /> Secure Lobby Instantiated
                    </div>
                    <div className="font-mono text-4xl md:text-5xl font-bold tracking-widest text-foreground bg-surface/40 border border-border/60 py-4 px-6 rounded-md inline-block select-all">
                      {generatedCode}
                    </div>
                    <p className="text-muted-foreground text-sm max-w-md mx-auto">
                      Provide this match key to the opposing side. To ensure complete fairness, your entry fee has been securely backed in your wallet.
                    </p>
                    
                    <div className="flex flex-col sm:flex-row gap-3 justify-center max-w-sm mx-auto pt-4">
                      <Button onClick={handleCopy} variant="outline" className="gap-2 bg-background/50 backdrop-blur flex-1 font-semibold card-lift transition-all duration-300 hover:border-primary/40">
                        {copied ? <CheckCircle2 className="size-4 text-success" /> : <Copy className="size-4" />}
                        {copied ? "Copied!" : "Copy Match Key"}
                      </Button>
                      <Button onClick={() => navigate({ to: "/match/$id", params: { id: createdMatchId } })} className="flex-1 gap-2 font-bold shadow-[var(--shadow-glow)] card-lift transition-all duration-300">
          ...
          <TabsContent value="join">
            <div className="rounded-lg border border-border bg-card p-8 shadow-[var(--shadow-card)] transition-all duration-300 hover:border-primary/40 group">
              <div className="max-w-md mx-auto text-center space-y-6">
                <QrCode className="size-12 text-primary mx-auto opacity-70 group-hover:scale-110 transition-transform duration-300" />
                <div>
                  <h2 className="font-display text-2xl font-bold tracking-tight">Enter Arena Key</h2>
                  <p className="text-sm text-muted-foreground mt-1">
                    Input the secure tournament lobby key generated by the match creator to audit details and join.
                  </p>
                </div>
                ...
                <form onSubmit={handleJoin} className="space-y-4 pt-2">
                  <div>
                    <Input
                      type="text"
                      placeholder="e.g. KABAOO-ABC12"
                      required
                      value={joinCode}
                      onChange={(e) => setJoinCode(e.target.value.toUpperCase())}
                      className="text-center font-mono text-xl uppercase tracking-widest py-6 border-border transition-all duration-300 hover:border-primary/40 focus-visible:border-primary"
                      maxLength={13}
                    />
                  </div>

                  {joinError && (
                    <div className="flex items-center justify-center gap-2 rounded-md border border-destructive/30 bg-destructive/10 p-3 text-sm text-foreground font-semibold">
                      <AlertCircle className="size-4 shrink-0 text-destructive" />
                      {joinError}
                    </div>
                  )}

                  <Button type="submit" size="lg" className="w-full gap-2 font-bold text-base h-12 shadow-[var(--shadow-glow)] card-lift transition-all duration-300" disabled={joinLoading || joinCode.trim().length < 5}>
                    {joinLoading ? <Loader2 className="size-5 animate-spin" /> : "Verify & Connect Arena"} <ArrowRight className="size-4" />
                  </Button>
                </form>
              </div>
            </div>
